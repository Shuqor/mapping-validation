...
BASELINE_PATH = Path(__file__).resolve().parent.parent / "results" / "stage9_x12_bridge_baseline.json"


def project_stage9_x12_bridge_report(report: dict) -> dict:
...
def test_stage9_x12_bridge_baseline_snapshot_matches_known_projection():
    ...
    assert projection == baseline_payload["projection"], (
        "Stage 9 X12 bridge baseline drift detected. If this change is intentional, "
        "update results/stage9_x12_bridge_baseline.json with the new projection."
    )
